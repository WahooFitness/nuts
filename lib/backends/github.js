...
GitHubBackend.prototype.getAssetStream = function(asset) {
    ...
    // Handle basic auth if username/password provided instead of token
    if (!this.opts.token && this.opts.username) {
        var credentials = Buffer.from(this.opts.username + ':' + this.opts.password).toString('base64');
        headers['Authorization'] = 'Basic ' + credentials;
    }

    return Q(fetch(asset.raw.url, fetchOptions).then(function(response) {
        if (!response.ok) {
