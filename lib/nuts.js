...
function Nuts(opts) {
    ...
    this.router.get('/download/channel/:channel/:platform?', this.onDownload.bind(this));
    this.router.get('/download/version/:tag/:platform?', this.onDownload.bind(this));
    ...
    this.router.get('/feed/channel/:channel.atom', this.onServeVersionsFeed.bind(this));

    this.router.get('/update', this.onUpdateRedirect.bind(this));
    this.router.get('/update/channel/:channel', this.onUpdateRedirect.bind(this));
    this.router.get('/update/channel/:channel/RELEASES', this.onUpdateRedirect.bind(this));
    this.router.get('/update/channel/:channel/:platform/:version', this.onUpdate.bind(this));
    this.router.get('/update/channel/:channel/:platform/:version/RELEASES', this.onUpdateWin.bind(this));
    this.router.get('/update/:platform/:version', this.onUpdate.bind(this));
    this.router.get('/update/:platform/:version/RELEASES', this.onUpdateWin.bind(this));
...
Nuts.prototype.onDownload = function(req, res, next) {
    ...
    .then(function(version) {
        ...
        if (filename) {
            asset = version.platforms.filter(platform => platform.filename.toLowerCase() === filename.toLowerCase())[0]
        } else {
        ...
        if (!asset) {
            throw createError(404, 'No download available for platform ' + platform
                + ' for version ' + version.tag + ' for filename ' + filename + ' (' + (channel || 'beta') + ')');
        }
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    var platform = req.params.platform;
    var channel = req.params.channel || determineChannel(req);
