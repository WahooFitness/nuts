...
/**
 * Merge duplicate versions (same tag) into single entries
 * This handles cases where S3 cache timing causes a release's assets
 * to be split across multiple entries
 * @param  {Array<Version>} versions
 * @return {Array<Version>} merged versions
 */
function mergeVersions(versions) {
    var byTag = {};

    _.each(versions, function(version) {
        if (!byTag[version.tag]) {
            byTag[version.tag] = version;
        } else {
            // Merge platforms, avoiding duplicates by asset id
            var existingIds = _.map(byTag[version.tag].platforms, 'id');
            _.each(version.platforms, function(platform) {
                if (!_.includes(existingIds, platform.id)) {
                    byTag[version.tag].platforms.push(platform);
                }
            });

            // Use the most recent published_at date
            if (version.published_at > byTag[version.tag].published_at) {
                byTag[version.tag].published_at = version.published_at;
            }

            // Merge notes if the existing one is empty
            if (!byTag[version.tag].notes && version.notes) {
                byTag[version.tag].notes = version.notes;
            }
        }
    });

    return _.values(byTag);
}
...
Versions.prototype.list = function() {
    ...
    .then(function(releases) {
        var versions = _.chain(releases)
            .map(normalizeVersion)
            .compact()
            .value();

        // Merge any duplicate versions (same tag) that may have been
        // split due to S3 cache timing during multi-platform uploads
        versions = mergeVersions(versions);
    ...
    .then(function(versions) {
        return _.chain(versions)
            .filter(function(version) {
                ...
                if (opts.platform && !platforms.satisfies(opts.platform, _.map(version.platforms, 'type'))) {
                    return false;
                }
...
Versions.prototype.resolve = function(opts) {
    ...
    .then(function(versions) {
        ...
        console.log("Channel: " + opts.channel + " Tag: " + opts.tag + " Platform: " + opts.platform);
        console.log("Version: " + version);
        // Matt's hack for failure that randomly appears, update version, platform and channel and uncomment when needed
        // if (_.first(versions)['tag'] == "7.98.2-beta.1" && opts.platform == "windows_32" && opts.channel == "beta") version = _.nth(versions, 1);
        if (!version) {
